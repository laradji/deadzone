...
func Open(path string) (*sql.DB, error) {
	...
	_, err = db.Exec(`CREATE VIRTUAL TABLE IF NOT EXISTS docs USING fts5(
		lib_id, title, content, tokenize="unicode61"
	)`)
	if err != nil {
...
func Insert(db *sql.DB, doc Doc) error {
	_, err := db.Exec(`INSERT INTO docs(lib_id, title, content) VALUES (?, ?, ?)`,
		doc.LibID, doc.Title, doc.Content)
	if err != nil {
...
// Search queries the FTS5 index. If libID is non-empty, results are filtered to that library.
func Search(db *sql.DB, query, libID string) ([]Doc, error) {
	var (
		rows *sql.Rows
		err  error
	)
	...
	if libID != "" {
		rows, err = db.Query(
			`SELECT lib_id, title, content FROM docs WHERE docs MATCH ? AND lib_id = ? ORDER BY rank`,
			query, libID,
		)
	} else {
		rows, err = db.Query(
			`SELECT lib_id, title, content FROM docs WHERE docs MATCH ? ORDER BY rank`,
			query,
		)
	}
	if err != nil {
